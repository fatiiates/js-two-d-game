...
function canvasOverMove(){
  ...
  isDown = false;
  onMovePiece != false ? onMovePiece.comeBack(tempData.area, tempData.moveX, tempData.moveY):'';
...
function canvasMouseMove(e){
  ...
  if (isDown && onMovePiece != false) {
    ...
    if (e.layerX + onMovePiece.radius+1 <= myGameArea.canvas.width && e.layerX - onMovePiece.radius -1 >= 0){
      onMovePiece.move.x = e.layerX;
      drag = true;
    }
    if (e.layerY + onMovePiece.radius +3 <= myGameArea.canvas.height && e.layerY - onMovePiece.radius - 5 >= 0){
    ...
    if (onMovePiece.drag) {
      ...
      mySling.forEach((item, i) => {
        item.collide(onMovePiece.real.x, onMovePiece.real.y, onMovePiece);
...
function autoLaunch(){
  ...
  // RENKLİ DUVARLARDAN YUKARISI(0 NOLU ALAN) BİLGİSAYARIN ALANI
  let pieceRand;

  do{
    pieceRand = Math.random() * myGamePiece.length;
    // rastgele seçilen topun alanı 1 ise tekrarlıyor, 0 ise devam ediyor
  }while(myGamePiece[pieceRand].area);
